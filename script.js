...
var ssIndex = 1;
function ssMain() {
    if (window.location.href !== "https://esale.ikco.ir/#!/searchcars?cca=0&csb=0&ccc=0&cpd=0") {
        return;
    }
    ...
    if (!findCarTarget) {
        setTimeout(function() {
            ssMain()
        }, 2000 + Math.floor(Math.random() * 90 + 10));
    }
}
...
shortcut.add("Ctrl+4",function() {
    if (new Date().getUTCDate() === ssd.getUTCDate() && new Date().getUTCMonth() === ssd.getUTCMonth() && new Date().getUTCFullYear() === ssd.getUTCFullYear()) {
